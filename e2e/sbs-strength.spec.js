...
test('onboarding protects the Excel prescription and opens the runner', async ({ page }) => {
  ...
  await expect(page.getByLabel('Peso Single @8 Squat')).not.toHaveValue('')
  await expect(page.getByLabel('Peso Serie 1 Squat')).not.toHaveValue('')
  await expect(page.getByLabel('Reps Serie 1 Squat')).toHaveValue('5')
  await expect(page.getByLabel('Reps Serie 5 AMRAP Squat')).toHaveValue('')
  await expect(page.getByRole('button', { name: 'Calibrar TM' })).toHaveCount(0)
})
...
test('autosaves main lifts, bodybuilding and conditioning in IndexedDB', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Calibrar TM' }).click()
  await expect(page.getByRole('button', { name: 'Calibrar TM' })).toHaveCount(0)
...
test('completion stores reliable active time and a persistent summary', async ({ page }) => {
  ...
  const completedId = await sessionIdFor(page, 'W1D1')
  await page.getByRole('link', { name: 'Calendario' }).click()
  await page.goto(`/resumen/${completedId}`)
...
test('analytics and protected settings are reachable by persistent routes', async ({ page }) => {
  await completeOnboarding(page)
  await expect(page.getByText('Peso corporal')).toHaveCount(0)
  await page.goto('/analiticas')
  await expect(page.getByRole('heading', { name: 'Analíticas' })).toBeVisible()
  await expect(page.getByLabel('Seleccionar lift')).toHaveValue('main_1')
  await expect(page.getByText('Tonelaje registrado')).toHaveCount(0)
  await expect(page.getByText('Peso corporal')).toHaveCount(0)
  const analyticsAccessibility = await new AxeBuilder({ page }).analyze()
  expect(analyticsAccessibility.violations.filter((violation) => ['serious', 'critical'].includes(violation.impact || ''))).toEqual([])
